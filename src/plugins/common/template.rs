//! Template substitution for plugin titles/bodies.
//!
...
//!
//! [`escape_for_osascript`] escapes `"` and `\` so a rendered string
//! can be spliced into an AppleScript string literal without breaking
//! the surrounding quotes. Used by the notifier's `Osascript` backend.
